...
local function midi_input_callback(packet_list, ref_conn, conn)
    for i = 0, tonumber(packet_list.numPackets) - 1 do
        if tonumber(packet_list.packet[i].length) ~= 3 then
            NSLog("Expecting a Midi Packet with length of 3")
            return
        end
        ...
        NSLog("MIDI data %@", data)

        if App.peripheral and App.characteristic then
            NSLog("Writing to characteristic %@", App.characteristic)
...
local function main()
    ...
    NSLog("Creating MIDI input port")
    ...
    NSLog("Connecting MIDI source to MIDI port")
